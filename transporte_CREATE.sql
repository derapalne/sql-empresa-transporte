...
BEGIN
    ...
    DECLARE total_factura_origen DECIMAL (9,2);
    DECLARE total_factura_destino DECIMAL (9,2);
    ...
    DROP TEMPORARY TABLE IF EXISTS `ganancias_estaciones`;
    CREATE TEMPORARY TABLE `ganancias_estaciones` (
		id_estacion INT NOT NULL PRIMARY KEY,
        nombre_estacion VARCHAR(40),
        ciudad_estacion VARCHAR(30),
        ganancias_origen DECIMAL(9,2) DEFAULT 0.00,
        ganancias_destino DECIMAL(9,2) DEFAULT 0.00
    );
    -- abrir cursor y loop
    OPEN curs;
    cursor_loop: LOOP
		...
		INSERT INTO ganancias_estaciones (id_estacion, nombre_estacion, ciudad_estacion)
		SELECT id_estacion, nombre_estacion, ciudad_estacion FROM estacion WHERE id_estacion=id_estacion_actual;
		-- total generado como estación de origen
		SET total_factura_origen = (SELECT SUM(f.total_factura)/2 FROM factura f
		...
		-- total generado como estación de destino
		SET total_factura_destino = (SELECT SUM(f.total_factura)/2 FROM factura f
									 JOIN transporte t ON f.id_transporte=t.id_transporte
									 WHERE t.id_estacion_destino=id_estacion_actual AND f.estado_factura="PAGA");
		
		-- verificar que el resultado no sea null (complica la visualización)
		IF total_factura_origen IS NULL THEN
			SET total_factura_origen = 0.00;
		END IF;
        IF total_factura_destino IS NULL THEN
			SET total_factura_destino = 0.00;
		END IF;
		...
		UPDATE ganancias_estaciones SET ganancias_origen = total_factura_origen, 
										ganancias_destino = total_factura_destino
